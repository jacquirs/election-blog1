...
National economic variables and their relationship to popular vote outcomes can help define the economic model of voting behavior. Specifically focusing on quarter two results in election years across of variety of economic variables-- quarter two based on research [compiled by Achen and Bartels](https://muse-jhu-edu.ezp-prod1.hul.harvard.edu/book/64646) on the retrospective model noting that recent events have more impact on voting decisions-- I will identify which predictors offer valuable insights for forecasting 2024 results. The variables to be examined are: 

_GDP:_ gross domestic product (billions)
_GDP Growth Q2:_ quarterly GDP growth in Q2
_RDPI:_ real disposable personal income (dollars)
_RDPI Growth Q2:_ quarterly RDPI growth in Q2
_CPI:_ consumer price index (dollars)
_Unemployment:_ unemployment rate (%)
_SP500:_ S&P 500 index close, open, high, low, adjusted close, volume (dollars)
_DPI:_ disposable personal income (dollars)

Based on in-class explorations of effective predictive economic fundamentals, I will begin with bivariate regression models using GDP growth and RDPI growth for Q2.
...
Analyzing the fit of these two bivariate regression models, neither shows strong results and in-sample fit. RDPI growth explains only 11.15% of the variance in incumbent popular vote share ($R^2 metric$), while GDP growth fares slightly better at 32.48%. Despite being the top predictors relatively in terms of in-sample fit, as I will show below, their overall performance does not indicate strong predictive power. 

Additionally, the correlation between these predictors and the popular vote outcomes (1952-2016) is moderately strong and positive: 0.569918 for GDP growth and 0.3338966 for RDPI growth. However, correlation does not imply causation, and there is little evidence of a direct causal relationship between these economic indicators on their own and popular vote share. Additional omitted variables and multivariate relationships likely play a role in the votes that I cannot control for. Notably, the models perform worse when including the 2020 economic outlier year.

A summary of the relationships of other national economic variables used as predictors is shown below.
...
Evaluating these models indicates that none are particularly strong in describing incumbent popular vote data over time or significant in their ability to predict future values for the 2024 election that indicate one winner over the other. All 13 regressions forecast Harris' popular vote percentage between 44.216% and 51.973%, but with 95% confidence intervals including 50%, indicating non-significant results. Additional factors are also needed to understand how Harris as the technical incumbent but not current president may fare within these models, a variable we cannot accurately reflect with these bivariate cases. 

Among the models, Q2 GDP growth in the election year performs the best across all model evaluations, showing the highest in-sample fit/$R^2$, smallest root mean squared error, and smallest mean absolute error in cross-validation for out-of-sample fit. The predictions for 2024 are also sensitive to the choice of predictor, suggesting that while economic fundamentals may influence incumbent vote outcomes, it may not be a direct bivariate relationship as the choice of predictor changes the model. This raises questions about whether economic fundamentals impact voting directly or indirectly and whether national (sociotropic) versus individual economic measures are more significant, an often explored phenomenon in the literature by scholars like [Gregory Markus of the University of Michigan](https://www.jstor.org/stable/2111314).


### State Level Predictors
While sociotropic voting suggests national economic variables influence incumbent vote share, I will also explore individual economic considerations. Some voters may focus on personal economic changes rather than national trends. To investigate this, I will examine state-level regressions using unemployment rates to see if they provide better predictive power than national data.
...
Visualizing the results from these 48 regressions above (excluding Hawaii, Alaska, and Washington DC due to data limitations) shows varied in-sample fit values similar to national economic fundamentals. While focusing on Q2 unemployment rate in states from elections 1976-2016, the correlation to incumbent vote-share varies-- stronger than national GDP growth was in states like North Dakota and nearly zero in states like Michigan.

Without state-level GDP growth readily available, I cannot test whether it performs better in sociotropic or individual settings. The state-by-state unemployment rate, with an average in-sample fit of 0.132, outperforms the national unemployment rate's near-zero variance but still falls short compared to national GDP growth. This is counter to in-class results, suggesting RDPI growth is a better predictor of incumbent popular vote share, which is not supported by these results.
...
Note that each regression has only twelve data points, most likely leading to model overfitting. This issue also affects the national economic fundamentals data even with its slightly larger dataset. The limited number of elections makes it challenging to develop a model with strong out-of-sample performance.


### Individual Versus Sociotropic Voting Patterns
The results suggest that sociotropic voting, where people base their choices on national economic conditions affecting others, is a more likely explanation in the retrospective economic voting model than individual-based voting. This aligns with findings on the importance of aggregate over individual conditions.

As noted by [Achen and Bartels in "Democracy for Realists: Why Elections Do Not Produce Responsive Government"](https://muse-jhu-edu.ezp-prod1.hul.harvard.edu/book/64646), voters may be retrospective but focus on short terms before elections in order to evaluate their choices, supporting the use of Q2 election-year metrics. [Scholars Lenz and Healy](https://www.journals.uchicago.edu/doi/abs/10.1086/692785) also that economic evaluations during election years only may favor economic manipulators over true leaders, providing insight into manipulations of voter decision-making. 

It is also important to consider how predictors and trends in incumbent popular vote may have changed over the period. Voters may have shifted away from retrospective behavior because parties have moved such that  people won’t flip between them anymore. Evidence from [Dassoneville and Tien's "Introduction to Forecasting the 2020 US Elections"](https://www.cambridge.org/core/journals/ps-political-science-and-politics/article/abs/introduction-to-forecasting-the-2020-us-elections/78235400F6BB7E2E370214D1A2307028) highlights that such changes and shocks to economic variables may throw off predictions, which is why I excluded 2020 data from the analysis.

Taken together, this exploration and analysis of the literature leads to me use national GDP growth in Q2 as the predictor for the 2024 forecast:

**Current Forecast: Harris 51.585% - Trump 48.415%**

with a lack of statistical significance.
