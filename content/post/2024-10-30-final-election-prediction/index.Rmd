...
Nine weeks ago I presented a simple model to predict the outcome of the election-- $ vote_{2024} = 0.75vote_{2020} + 0.25vote_{2016}$. While this simple weighted average may seem to over-simply the complex process that is the presidential election, as discussed over the past blog posts it is incredibly important to carefully evaluate all elements of a model. The most complicated model may not always be the best, as parsimony can be key and overfitting can be fatal. 
...
$$
\quad D_{\text{pv2p}} = \beta_0 + \beta_1 * D_{\text{pv2p_lag1}} + \beta_2 * D_{\text{pv2p_lag2}} + \beta_3 * \text{latest_pollav_DEM} + \beta_4 * \text{mean_pollav_DEM} 
$$
$$
+ \beta_5 * \text{vep_turnout} + \beta_6 * \text{GDP_growth_quarterly} + \beta_7 * \text{RDPI_growth_quarterly}
$$
$$
+ \beta_8 * \text{GDP_growth_quarterly} * \text{incumbency} + \beta_9 * \text{RDPI_growth_quarterly} * \text{incumbency} + \epsilon
$$



$$
\quad R_{\text{pv2p}} = \alpha_0 + \alpha_1 * R_{\text{pv2p_lag1}} + \alpha_2 * R_{\text{pv2p_lag2}} + \alpha_3 * \text{latest_pollav_REP} + \alpha_4 * \text{mean_pollav_REP} 
$$
$$
+ \alpha_5 * \text{vep_turnout} + \alpha_6 * \text{GDP_growth_quarterly} + \alpha_7 * \text{RDPI_growth_quarterly}
$$
$$
+ \alpha_8 * \text{GDP_growth_quarterly} * \text{incumbency} + \alpha_9 * \text{RDPI_growth_quarterly} * \text{incumbency} + \epsilon
$$
...
$$
$$
+ \beta_6 * \text{mean_pollav_REP} + \beta_7 * \text{vep_turnout} + \beta_{8} * \text{GDP_growth_quarterly} + \beta_{9} * \text{RDPI_growth_quarterly} 
$$
$$
+ \beta_{10} * \text{GDP_growth_quarterly} *\text{incumbency}} + \beta_{11} * \text{RDPI_growth_quarterly} * \text{incumbency} + \epsilon
$$
