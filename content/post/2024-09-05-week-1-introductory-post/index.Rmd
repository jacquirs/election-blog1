...
This week's focus is on data from national presidential election outcomes, 1948-2020. Before analyzing the factors leading to a candidate's victory, it is important to understand past election trends. The objective of this post is to answer two questions in furtherance of this goal: **(a) how competitive are presidential elections in the United States**, and **(b) which states vote blue/red and how consistently**.  To do so, I'll examine Electoral College and two-way popular vote results, margins, and swings, and implement a simplified Helmut Norpoth model for a baseline prediction based solely on prior election results.

### Popular Vote Over Time
The national popular vote provides the percentage of all votes cast for a specific candidate. A more focused analysis looks at the two-way popular vote, considering only votes cast for the two main parties. Looking at the two-way popular vote results 1948-2020, I can evaluate how the popular vote distribution has changed between the parties.
...
From 1948 to 2020, neither party has consistently dominated. Power shifts between Democrats and Republicans occur every few election cycles. The margins have varied significantly, with some elections resulting in large gaps, like the 23.6 percentage-point difference in 1972 between Nixon and McGovern, and others with extremely tight races, such as the 0.17 percentage-point margin in 1960 between Nixon and Kennedy. Over the decades, landslide victories have seemingly become less common, with margins still fluctuating but within consistently tighter races. 
...
The dominance of one party or the other in the Electoral College has similarly declined, with fewer landslide victories in recent years. This points to an _increased competitiveness in presidential elections_.
...
As seen in 2000 (and 2016), observed fluctuations in both electoral and popular vote results are of similar magnitude, but may lead to the same outcome. _These cases demonstrate how the national popular vote and Electoral College results don’t always align, underscoring the importance of state-level popular vote outcomes in predicting elections._
...
For instance, Utah consistently votes Republican but saw a red swing in 2012 when Mitt Romney, [a son of Utah](https://link.springer.com/chapter/10.1057/9781137360823_5), was the candidate. This greatly affected the swing in adjacent presidential elections, making 2016 blue with its smaller win-margin. This does not make Utah a swing state, as it has not voted for a Democrat since President Johnson's 1964 landslide.

By contrast, states with consistently smaller swings, like Pennsylvania and Arizona, are more likely to be true swing states. It is important to look both at the swing and the actual popular vote to see which states are actually swing states. A swing map visualizes only how much states move relatively. For example, in 2008, many states swung more Democratic than the past for Obama, but in 2012, the swing shifted red. However, Obama still won re-election, demonstrating that while swing maps show support movement, the popular vote within each state matters.

### The Helmut Norpoth Model
While swings and margins provide valuable information, they alone are not enough to predict outcomes. The same goes for the popular vote nationally, which does not always determine the electoral votes. But looking at state-level popular vote can provide an early 2024 election forecast based only on past results using a simplified version of the [Helmut Northpoth electoral cycle model](http://primarymodel.com/). This is defined by 

$$ vote_{2024} = 0.75vote_{2020} + 0.25vote_{2016}$$
This is a _predictive model_ of the election, using inputs of past election results to predict a future outcome. The map generated by this model shows predicted margins of victory by state, as well as the projected winner and total electoral votes.
...
Most states are projected to swing slightly more Republican in 2024 than in 2020, but this does not indicate a Republican victory. In fact, this makes sense for the simplified model, which relies 75% on the 2020 results and 25% on the 2016 results in each state. In 2016, many states voted more Republican than they did in 2020, so using these years automatically pulls the 2024 swings to the Republicans.
