...
knitr::opts_knit$set(root.dir = "C:/Users/Jacqui Schlesinger/Documents/election-blog1/Week 7")
...
```
who is the voting eligible population
...
```
harris beats trump when taking into account the sd of the polls historically, may want to look at throughout 2024 versus week 3 in the past 
...
# Clinton 2016 Field Offices - Obama 2012 Field Offices. 
fo_add %>% 
  filter(candidate == "Obama", year == 2012) %>%
  summarize(n = n())

fo_add %>% 
  filter(candidate == "Romney", year == 2012) %>%
  summarize(n = n())

fo_add %>% 
  filter(candidate == "Clinton", year == 2016) %>%
  summarize(n = n())

fo_add %>% 
  filter(candidate == "Trump", year == 2016) %>%
  summarize(n = n())

# Trump and Clinton by State
fo_add %>% 
  filter(candidate == "Trump", year == 2016) %>%
  group_by(state) %>%
  summarize(n = n())
...
fo_add %>% 
  filter(candidate == "Obama", year == 2012, state == "WI") %>%
  group_by(state) %>%
  summarize(n = n())

fo_add %>% 
  filter(candidate == "Clinton", year == 2016, state == "WI") %>%
  group_by(state) %>%
  summarize(n = n())
